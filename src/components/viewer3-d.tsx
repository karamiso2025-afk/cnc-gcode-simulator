...
function ToolSweptPath(_props: ToolSweptPathProps): JSX.Element | null {
  ...
  return null;
  const radius = _props.toolDiameter / 2;

  // Build quads along cutting paths; shader renders perfect circles
  // depthWrite=true prevents overlapping same-Z circles from stacking opacity
  const geometry = useMemo(() => {
    if (!showTool || radius <= 0 || segments.length === 0) return null;

    const visibleCutting = segments.slice(0, currentSegmentIndex + 1).filter(s => s.type !== 'rapid');
    if (visibleCutting.length === 0) return null;

    const conv = xyPlaneView ? toVec3Flat : toVec3;
    const step = Math.max(radius * 0.3, 0.5);
    ...
    const addQuad = (p: Point3D): void => {
      const r = radius;
      const p0 = conv({ x: p.x - r, y: p.y - r, z: p.z });
      const p1 = conv({ x: p.x + r, y: p.y - r, z: p.z });
      const p2 = conv({ x: p.x + r, y: p.y + r, z: p.z });
      const p3 = conv({ x: p.x - r, y: p.y + r, z: p.z });
    ...
    for (const seg of visibleCutting) {
    ...
    const geom = new THREE.BufferGeometry();
    geom.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
    geom.setAttribute('localUV', new THREE.Float32BufferAttribute(uvs, 2));
    geom.setIndex(indices);
    return geom;
  }, [segments, currentSegmentIndex, radius, showTool, xyPlaneView]);

  if (!geometry) return null;

  return <mesh geometry={geometry} material={sweptCircleMaterial} renderOrder={2} />;
}
